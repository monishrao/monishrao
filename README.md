...
* ### dec_convert function:

	__init__ function passes it's input here.
	The input is passed, randomly selects a number between -1,1
	It then gets converted to a Decimal, which inturn is multiplied it with the input, and rounded off to 10th decimal place
	This final result is sent back to __init__ function.
